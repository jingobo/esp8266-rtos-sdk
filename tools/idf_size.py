...
def print_summary(memory_config, sections):
    ...
    def get_address(section):
        try:
            return sections[section]["address"]
        except KeyError:
            return 0

    iparts = { "irom0_0_seg" : "FLASH", "iram1_0_seg" : "IRAM", "dram0_0_seg" : "RAM " }
    for ss in iparts:
        # check start and end address
        top = memory_config[ss]["origin"]
        size = memory_config[ss]["length"]
        bottom = top + size
        # calculate used bytes
        used = 0;
        for ds in sections:
            if get_address(ds) >= top and get_address(ds) < bottom:
                used += get_size(ds)
        # output
        print("%s: %7d bytes (%7d available, %.1f%% used)" %
              (iparts[ss], used, size - used, 100.0 * used / size))
